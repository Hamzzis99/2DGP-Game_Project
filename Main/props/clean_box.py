...
class Clean_box(GameObject):
    image = None  # 클래스 변수로 이미지 로드 공유

    def __init__(self, x, y):
        if Clean_box.image is None:
            Clean_box.image = load_image('img/tiles.png')  # Items 이미지 로드
            # Clean_box.image.set_color_key((255, 255, 255))  # 흰색을 투명하게 설정 (필요 시)
        self.x = x
        self.y = y
    ...
    def draw(self):
        if self.changed:
            ...
            # 애니메이션 상태일 때 이미지 그리기
            adjusted_sprite_y = Clean_box.image.h - self.sprite_y - self.height
            sprite_x = self.sprite_x_positions[self.frame]
            Clean_box.image.clip_draw(
    ...
    def get_top_bb(self):
        # Clean_box 상단의 충돌 박스 (좌우로 1픽셀씩 줄이고 y 범위도 축소)
    ...
    def get_bottom_bb(self):
        # Clean_box 하단의 충돌 박스 (좌우로 1픽셀씩 줄이고 y 범위도 축소)
        half_width = (self.width * self.scale) / 2 - 1
        return self.x - half_width, self.y - (self.height * self.scale) / 2 - 2, \
               self.x + half_width, self.y - (self.height * self.scale) / 2 + 2
    ...
    def get_left_bb(self):
        # Clean_box 왼쪽의 충돌 박스 (위아래로 1픽셀씩 줄임)
        half_width = (self.width * self.scale) / 2
    ...
    def get_right_bb(self):
        # Clean_box 오른쪽의 충돌 박스 (위아래로 1픽셀씩 줄임)
        half_width = (self.width * self.scale) / 2
        half_height = (self.height * self.scale) / 2 - 1  # 상하로 1픽셀씩 줄임
    ...
    def handle_collision(self, group, other, hit_position, hit_side=None):
        ...
        """
        if not self.changed and group.startswith('mario:clean_box'):
            ...
            # 점수 텍스트 생성
            score_text = ScoreText(self.x, self.y + self.height * self.scale / 2 + 20, "+100")  # 문자열 전달
            game_world.add_object(score_text, 1)
