...
class Mushroom_box(GameObject):
    image = None  # 클래스 변수로 이미지 로드 공유

    def __init__(self, x, y):
        if Mushroom_box.image is None:
            Mushroom_box.image = load_image('img/tiles.png')  # Items 이미지 로드
        self.x = x
        self.y = y
        ...
        # 애니메이션 관련 변수
        self.sprite_x_positions = [384, 400, 416]  # 애니메이션 프레임 x 좌표들 (필요시 수정)
        self.frame = 0
    ...
    def draw_with_camera(self, camera: Camera):
        ...
        if not self.changed:
            ...
            sprite_x = 432  # 상태 변경 후에는 동일한 스프라이트 사용 (필요시 수정)
    ...
    def get_top_bb(self):
        # Mushroom_box 상단의 충돌 박스 (좌우로 1픽셀씩 줄이고 y 범위도 축소)
    ...
    def get_bottom_bb(self):
        # Mushroom_box 하단의 충돌 박스 (좌우로 1픽셀씩 줄이고 y 범위도 축소)
        half_width = (self.width * self.scale) / 2 - 1
        return (
            self.x - half_width,
            self.y - (self.height * self.scale) / 2 - 2,
    ...
    def get_left_bb(self):
        # Mushroom_box 왼쪽의 충돌 박스 (위아래로 1픽셀씩 줄임)
        half_width = (self.width * self.scale) / 2
    ...
    def get_right_bb(self):
        # Mushroom_box 오른쪽의 충돌 박스 (위아래로 1픽셀씩 줄임)
        half_width = (self.width * self.scale) / 2
        half_height = (self.height * self.scale) / 2 - 1  # 상하로 1픽셀씩 줄임
        return (
            self.x + half_width,
