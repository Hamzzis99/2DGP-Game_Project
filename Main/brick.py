...
class Brick:
    ...
    def get_top_bb(self):
        # Brick 상단의 충돌 박스 (좌우로 1픽셀씩 줄임)
        half_width = (self.width * self.scale) / 2 - 1
        return self.x - half_width, self.y + (self.height * self.scale) / 2 - 4, self.x + half_width, self.y + (self.height * self.scale) / 2 + 4

    def get_bottom_bb(self):
        # Brick 하단의 충돌 박스 (좌우로 1픽셀씩 줄임)
        half_width = (self.width * self.scale) / 2 - 1
        return self.x - half_width, self.y - (self.height * self.scale) / 2 - 4, self.x + half_width, self.y - (self.height * self.scale) / 2 + 4
    ...
    def get_left_bb(self):
        ...
        half_width = (self.width * self.scale) / 2
        half_height = (self.height * self.scale) / 2 - 1
